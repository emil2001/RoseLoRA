...
from .roselora_model import RoseLoraModel
...
def execute_roselora(
    ...
    if not keep_original_weight and hasattr(model,'peft_config'):
        ...
        peft_model = RoseLoraModel(model, peft_config, adapter_name="default", total_step = hparams.num_steps*hparams.batch_size, sparsity=0.1)
    ...
    for it in range(hparams.num_steps):
        ...
        for txt, tgt in zip(
            ...
            # RoseLora step
            peft_model.update_and_allocate(iteration)
            iteration += 1
